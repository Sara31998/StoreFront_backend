...
class ProductStore {
    ...
    async createProduct(pro) {
        try {
            // the sql to write in the database
            const sqlcreate = 'INSERT INTO productstore (name_producr, descount_product,brand_product , price_product, category_product) VALUES($1, $2, $3, $4, $5) RETURNING *';
            // @ts-ignore
            // connect with the database to open
            const connection = await index_database_1.default.connect();
            // the result of the connect the database with the sql
