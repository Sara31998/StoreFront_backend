...
class OrderStore {
    ...
    async createOrder(ord) {
        try {
            ...
            const sqlcreate = 'INSERT INTO orderfromstore (id_user, status_order, quantity_order) VALUES($1, $2, $3) RETURNING *';
            // @ts-ignore
            // connect with database 
            const connection = await index_database_1.default.connect();
            // the result of connect the sql with database
    ...
    async deleteOrder(idord) {
        try {
            // @ts-ignore
            // connect with database 
            const conn = await index_database_1.default.connect();
    ...
    async addProductToOrder(pro_ord) {
        try {
            // the sql of the get product to  orders from stors
            const sqladdpro = 'INSERT INTO oderofproduct (orderstore, productstore) VALUES($1, $2) RETURNING *';
            // @ts-ignore
            // connect with database 
            const connection = await index_database_1.default.connect();
            // the result of connect the sql with database
