...
const user_store_models = new user_table_moddles_1.default();
// the vriables to used in test of order models
const oroduct_store_models = new order_table_modles_1.default();
...
describe('test the endpoint of the Order Models', () => {
    ...
    describe('test of the order medole ', () => {
        ...
        beforeAll(async () => {
            ...
            user_owner_ord.id = New_Client.id;
            // the order is the same id \
            order_test.id_user = user_owner_ord.id;
            // create the new order
            const new_oder = await oroduct_store_models.createOrder(order_test);
        ...
        it('test of the method of create the order if order not exite', async () => {
            ...
            const neworder = {
                quantity_order: 4,
                status_order: 'comblited',
                id_user: user_owner_ord.id, // the id of the user
            };
            // create new order
            const newordertocreate = await oroduct_store_models.createOrder(neworder);
        ...
        it('thest method of get one on order models', async () => {
            ...
            const ordergetone = await oroduct_store_models.getOrderById(order_test.id);
