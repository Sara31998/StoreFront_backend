...
// import the product type to test this
...
import Product_Store_Mod from '../product_table_modlles';

// the vriables to used in test
const product_store_models = new Product_Store_Mod()
...
describe('test the endpoint of the product', () => {
  ...
  beforeAll(async () => {
    ...
    const new_product = await product_store_models.createProduct(
      productstore1
    )
  ...
  describe('testing of the product models of methods loging', () => {
    it('if the login is create methods should return product', async () => {
      ...
      const productStore2 = await product_store_models.createProduct(
        newproductstore
      )
      ...
      expect(productStore2.category_product).toBe(
        newproductstore.category_product
      )
    ...
    it('the methods of the get one to return on product', async () => {
      ...
      const productgetone = await product_store_models.getProductById(
        productstore1.id
      )
      ...
      expect(productgetone.category_product).toBe(
    ...
    it('method f the update one of the product when login', async () => {
      ...
      const productsupdate = await product_store_models.updateProduct({
        ... productstore1,
         name_producr: 'blouse', // the th name of product
      ...
      // check if the producr_category is found
      expect(productsupdate.category_product).toBe(
        productstore1.category_product
      )
      // check if the product brand  is found
